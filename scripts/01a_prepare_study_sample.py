...
non_rDC_TIL_values = formatted_TIL_values[formatted_TIL_values.GUPI.isin(gupis_with_nonrefract_DecomCranectomy)].reset_index(drop=True)

non_rDC_TIL_values['reCalcTILBasic'] = np.where(non_rDC_TIL_values[['TILSedationMetabolic','TILHyperventilationIntensive','TILFeverHypothermia','TILICPSurgery']].sum(axis=1)>0, 4,
                                           np.where(non_rDC_TIL_values[['TILCSFDrainageHighVolume','TILHyperventilationModerate','TILMannitolHighDose','TILHypertonicSalineHighDose','TILFeverMildHypothermia']].sum(axis=1)>0, 3,
                                                    np.where(non_rDC_TIL_values[['TILSedationHigher','TILCSFDrainageLowVolume','TILFluidLoading','TILFluidLoadingVasopressor','TILHyperventilation','TILMannitolLowDose','TILHypertonicSalineLowDose']].sum(axis=1)>0, 2,
                                                             np.where(non_rDC_TIL_values[['TILPosition','TILPositionNursedFlat','TILSedation']].sum(axis=1)>0, 1,0))))

DCs = non_rDC_TIL_values[non_rDC_TIL_values.TILICPSurgeryDecomCraniectomy==1].reset_index(drop=True)
disconcordant = non_rDC_TIL_values[non_rDC_TIL_values.reCalcTILBasic!=non_rDC_TIL_values.TILBasic].reset_index(drop=True)