...
compiled_val_bootstrapping_metrics = pd.concat([TILBasic_compiled_val_orc,compiled_val_somers_d,TILBasic_compiled_val_calibration],ignore_index=True)
...
# Extract optimal-configuration TILBasic ORC per window index
...
# Merge TILBasic post-dropout tuning grids and save
post_dropout_tuning_grid = pd.concat([TILBasic_filt_tuning_grid],ignore_index=True).sort_values(by=['REPEAT','FOLD','TUNE_IDX'],ignore_index=True)
