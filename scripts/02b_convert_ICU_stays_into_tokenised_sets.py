...
### Contents:
# I. Initialisation
# II. Partition patient ICU stays into time windows
# III. Tokenise optional time variables
# IV. Load and prepare formatted categorical tokens
# V. Tokenise numeric variables and place into study windows
...
def main():
    ...
    windows_per_core = [(study_tokens_df.iloc[start_idx[idx]:end_idx[idx],:].reset_index(drop=True),True,'Cleaning categorical variable dataframe') for idx in range(len(start_idx))]
    ...
    ## Iterate through folds and tokenise numeric variables per distributions in the training set
    for curr_part_idx in tqdm(range(uniq_partitions.shape[0]),'Iterating through partitions of repeated cross-validation for numeric variable tokenisation'):

        ## Extract current repeated cross-validation parameters
        # Current repeat
        curr_repeat = uniq_partitions.REPEAT[curr_part_idx]

        # Current fold
        curr_fold = uniq_partitions.FOLD[curr_part_idx]
        ...
        end_idx = np.cumsum(s)
        start_idx = np.insert(end_idx[:-1],0,0)
        windows_per_core = [(cleaned_study_tokens_df.iloc[start_idx[idx]:end_idx[idx],:].reset_index(drop=True),True,'Cleaning token dataframe for repeat '+str(curr_repeat)+' fold '+str(curr_fold)) for idx in range(len(start_idx))]
